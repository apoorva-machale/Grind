...
class PromptManager:
    ...
    def compile(self) -> Tuple[bool, ChatPromptTemplate, str]:
        ...
        try:
            ...
            # SystemMessage = static text (few-shot JSON uses { } and must not be templated).
            # ("human", ...) tuple = {user_input} substituted at invoke time.
            # HumanMessage(content="{user_input}") leaves the literal "{user_input}" for the model.
            self.template = ChatPromptTemplate.from_messages([
                SystemMessage(content=system_message_content),
                (
                    "human",
                    "Customer question (answer this query specifically, in valid JSON only):\n{user_input}",
                ),
            ])
